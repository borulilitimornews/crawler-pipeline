...
class GetFinalCorpus:
    ...
    def is_text_to_filter(self, text_line: str) -> bool:
        ...
        :param start_patterns: the patterns to be verified in the initial words.
        :param end_patterns: the patterns to be verified in the final words.
        :param in_patterns: the patterns to be verified in the words.
        :return: True if the given text meets, False otherwise.
        """
        text_to_filter = any(
            text_line.lower().startswith(start_pattern) or
            text_line.lower().endswith(end_pattern) or
            in_pattern in text_line.lower()
    ...
    def filtered_text(self, initial_corpus_file_path: Path, final_corpus_file_path: Path,
            ...
            for line in f:
                ...
                if line not in seen_sentences:
                    if line == '':
                        ...
                        consecutive_newlines = 0
                    # Ensure that the maximum consecutive newlines are only 2.
                    if line == '' and consecutive_newlines >= max_consecutive_newlines:
                        continue
                    else:
                        unique_sentences.append(line)
                        # Do not add newline (empty line) to seen_sentence set.
                        if not line == '':
